...
export default function ProfilPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5] py-8 w-full overflow-hidden">
      <div className="w-full max-w-5xl mx-auto flex flex-col md:flex-row gap-6 px-4">
        
        {/* Sidebar Kiri (Opsional, ala Shopee) */}
        <div className="w-full md:w-64 hidden md:block shrink-0">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div className="overflow-hidden">
              <p className="font-bold text-gray-800 text-sm truncate w-full">{formData.name}</p>
        ...
        {/* Konten Utama Kanan */}
        <div className="flex-1 bg-white rounded-sm shadow-sm border border-gray-100 p-4 md:p-8 w-full">
          ...
          {/* Di HP: flex-col-reverse (Foto Profil duluan di atas), Di Laptop: flex-row (Kiri Form, Kanan Foto) */}
          <div className="flex flex-col-reverse md:flex-row gap-8 md:gap-10 w-full">
            
            {/* Form Kiri */}
            <div className="flex-1 space-y-4 w-full">
              
              <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-5 w-full">
                <label className="w-full md:w-32 md:text-right text-sm text-gray-500 shrink-0">Email</label>
                <div className="text-sm text-gray-800 font-medium w-full">{formData.email} <span className="text-[#B07D60] ml-2 text-xs cursor-pointer hover:underline">Ubah</span></div>
              </div>

              <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-5 w-full">
                <label className="w-full md:w-32 md:text-right text-sm text-gray-500 shrink-0">Nama</label>
                <input type="text" name="name" value={formData.name} onChange={handleChange} className="w-full border border-gray-300 rounded-sm px-3 py-2 text-sm focus:border-[#B07D60] focus:outline-none text-gray-900 bg-white" />
              </div>

              <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-5 w-full">
                <label className="w-full md:w-32 md:text-right text-sm text-gray-500 shrink-0">Nomor Telepon</label>
                <input type="text" name="phone" value={formData.phone} onChange={handleChange} placeholder="08xxxxxxxx" className="w-full border border-gray-300 rounded-sm px-3 py-2 text-sm focus:border-[#B07D60] focus:outline-none text-gray-900 bg-white" />
              </div>

              <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-5 w-full">
                <label className="w-full md:w-32 md:text-right text-sm text-gray-500 shrink-0">Nama Toko</label>
                <input type="text" name="store_name" value={formData.store_name} onChange={handleChange} placeholder="Toko Kreatifku" className="w-full border border-gray-300 rounded-sm px-3 py-2 text-sm focus:border-[#B07D60] focus:outline-none text-gray-900 bg-white" />
              </div>

              <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-5 w-full">
                <label className="w-full md:w-32 md:text-right text-sm text-gray-500 shrink-0">Jenis Kelamin</label>
                <div className="flex flex-wrap gap-4 text-sm text-gray-700 w-full">
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-5 w-full">
                <label className="w-full md:w-32 md:text-right text-sm text-gray-500 shrink-0">Tanggal Lahir</label>
                <input type="date" name="birth_date" value={formData.birth_date} onChange={handleChange} className="w-full md:w-auto border border-gray-300 rounded-sm px-3 py-2 text-sm focus:border-[#B07D60] focus:outline-none text-gray-900 bg-white" />
              </div>

              <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-5 pt-4 w-full">
                <div className="hidden md:block w-32 shrink-0"></div>
                <button onClick={handleSave} className="w-full md:w-auto bg-[#B07D60] text-white px-8 py-2.5 rounded-sm text-sm font-medium hover:bg-[#8D6E63] transition-colors">Simpan</button>
              </div>

            </div>

            {/* Upload Foto Kanan (Akan muncul di atas form kalau di HP) */}
            <div className="w-full md:w-64 md:border-l border-gray-100 flex flex-col items-center justify-center py-4 md:p-6 shrink-0">
              ...
              <label className="bg-white border border-gray-300 text-gray-900 px-4 py-2 rounded-sm text-sm font-medium cursor-pointer hover:bg-gray-50 transition-colors shadow-sm">
                Pilih Gambar
